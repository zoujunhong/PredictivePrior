...
from model.DINO_pointpred2 import DINOPredictor as Model
...
def train(segmentation_module, data_loader, optimizers, epoch, gpu, lr_schedule):
    ...
    for idx,data in enumerate(data_loader):
        ...
        # forward pass
        loss_1 = segmentation_module(dino_feat)
        loss_total = loss_1
        
        # # Backward
        loss_total.backward()
        # nn.utils.clip_grad_norm_(segmentation_module.parameters(), max_norm=1.0)
        optimizers.step()
        ...
        if dist.get_rank()==0:
            print('[{}][{}/{}], lr: {:.2f}, '
                  'time: {:.2f}, '
                  'Loss: {:.3f}'
                  .format(epoch, idx, epoch_iters, lr_schedule[it]*1000, batch_time.average(),
                  ave_loss_1.average()))
...
def main(gpu,args):
    ...
    # Main loop
    # checkpoint(segmentation_module, optimizer, args, 0)
    for epoch in range(args.resume_epoch, args.total_epoch):
        print('Epoch {}'.format(epoch))
        train(model, loader_train, optimizer, epoch, load_gpu, lr_schedule)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--saveroot",type=str,default='/root/onethingai-tmp/savemodel/MOVi_pointpred2')
